...
5.  make -j4 -DBUILD_TESTING=0

6.  The build will be completed at this point, executable is bin/
...
2.  Add a data member for the new item in the config.hpp file

3.  In the config.cpp file locate the if/else structure that repeats. Add in a similar structure for the new item such that if the value is in the config it is set to that new value, and if not it is set to the default specified in step 1. 

4.  Add the value to the json with the name you specified in step 3. 
...
For more information on [plotyl](https://plot.ly/python/getting-started/). For more information on [pandas](https://pandas.pydata.org/pandas-docs/stable/getting_started/install.html).
